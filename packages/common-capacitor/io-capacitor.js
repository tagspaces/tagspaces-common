 ...
 * Replaces io-cordova.js with Capacitor native APIs
 */
...
function getFileContentPromise(param, type, isPreview) {
  ...
  if (isPreview) {
    return Promise.resolve("Previewing files is not supported on this platform");
  }
...
function renameFilePromise(param, newFilePath, onProgress = undefined) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      console.log(
        "File renamed to: " + newFilePath + " Old name: " + filePath,
      );
...
function moveDirectoryPromise(param, newDirPath, onProgress = undefined) {
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      reject(
        "error moving directory: " + path + " " + JSON.stringify(err),
      );
    }
  });
}
